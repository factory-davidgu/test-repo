# Droid Custom Agent - Exact Prompt Received

This document contains the exact prompt/instructions that are passed to the custom agent when the "droid" agent is invoked.
...
The above content represents the exact instructions/prompt that is provided to this custom agent when it is invoked. This prompt:

1. Defines the agent's identity as a "Droid CLI assistant"
2. Specifies the agent's purpose (helping developers install and use Droid CLI)
3. Provides comprehensive documentation about Droid CLI installation and usage
4. Details various use cases, examples, and integration patterns
5. Includes security best practices and troubleshooting guidance
6. Explains how the agent integrates with GitHub Copilot's environment
7. Describes the tools available to the agent (read, search, edit, shell)

This prompt is what shapes the agent's behavior and knowledge when responding to user queries about the Droid CLI.
